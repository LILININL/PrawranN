...
import { Box, FormControl, Heading, HStack, Input, Text } from "native-base";
import React, {
  useCallback,
  useEffect,
  useMemo,
...
function InputSelect({
  ...
  const handlePickerSubmit = useCallback(() => {
    let selectedItem = options.find(
      (_item) => _item.value === selectedPickerValue
    );
      ...
      <Box px={2}>
        ...
        >
          <Box
            ...
            py={3}
            backgroundColor={
              item.value === selectedPickerValue ? "#eee" : "transparent"
            }
            borderRadius="4px"
          >
          ...
          return Platform.OS === "ios" ? (
            ...
            >
              <Box>
                <TouchableOpacity
                  activeOpacity={0.8}
                  onPress={closePickerModalHandle}
                >
                  <Text>ยกเลิก</Text>
                </TouchableOpacity>
              </Box>
      ...
      >
        {Platform.OS === "ios" ? (
          ...
          <BottomSheetFlatList
            ...
            initialScrollIndex={
              selectedPickerIndex > 0 ? selectedPickerIndex : 0
            }
