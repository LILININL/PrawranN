...
function LanguageSwitcher() {
  ...
  const [languageList] = useState<ILangOption[]>([
    {
      ...
      code: 'th',
      value: '+66',
      image: require('../image/flags/th.png'),
    },
    {
      label: 'ลาว',
      code: 'la',
      value: '+856',
      image: require('../image/flags/la.png'),
    },
    {
      label: 'พม่า',
      code: 'mm',
      value: '+95',
      image: require('../image/flags/mm.png'),
    },
    {
      label: 'กัมพูชา',
      code: 'kh',
      value: '+855',
      image: require('../image/flags/kh.png'),
    },
    {
      label: 'เวียดนาม',
      code: 'vn',
      value: '+84',
      image: require('../image/flags/vn.png'),
    },
    {
      label: 'มาเลเซีย',
      code: 'my',
      value: '+60',
      image: require('../image/flags/my.png'),
    },
  ]);

  // initialize selected language
  const [currentLanguage, setCurrentLanguage] = useState<ILangOption>(languageList[0]);

  useEffect(() => {
    // find current language
    const _currentLanguage = languageList.find((item) => item.code === i18n.language);
  ...
  const renderFlatListItem = useCallback(
    ({ item }) => (
      <Box px={4} backgroundColor={currentLanguage?.code === item.code ? 'gray.50' : 'white'}>
        <TouchableOpacity activeOpacity={0.8} onPress={() => changeLanguage(item)}>
          <HStack alignItems="center" justifyContent="space-between" py={3}>
            ...
            <Center
              width={6}
              height={6}
              backgroundColor={currentLanguage?.code === item.code ? 'green.500' : 'gray.100'}
              borderRadius="full">
              <UtilFontAwesome icon={faCheck} color="white" size={14} />
            </Center>
          </HStack>
        </TouchableOpacity>
      </Box>
    ...
    <Box>
      <Button
        ...
        <HStack flexDirection="row" alignItems="center" space={2}>
          <Image style={{ width: 22, height: 22 }} source={currentLanguage?.image} />
